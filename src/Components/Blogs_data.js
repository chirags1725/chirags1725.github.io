const BlogData = [
  {
    ...
    desc: "By default, the Python Eel library utilizes the Google Chrome web browser in order to run the application; however, we can also mention the browser with the help of the 'mode' option. Along with 'mode', there are a few other application choices available like 'position', 'size', 'geometry', and a lot more, which are passed inside the eel.start() command. To start, Install(as 'pip install eel') and Import eel module(as import eel). Then in the working directory, pass command eel.init(). Then pass command eel.start()",
    image: "",
  },
  {
    id: 2,
    Title: "Python Module: \"Wikipedia\"",
    desc: "We can now import the entire Wikipedia! Yes, We can now import Wikipedia in Python using the Wikipedia module. Use the incessant flow of knowledge with Python for daily needs. Install it as:'pip install wikipedia'. To use this module, import it as 'import wikipedia'. Then make a variable as variable_name = wikipedia.page(\"topic\"). Then print as 'print(variable_name.summary)'.",
    image: "3",
  },
  {
    id: 3,
